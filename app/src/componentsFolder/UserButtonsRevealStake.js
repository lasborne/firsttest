...
//This function defines the key buttons present on the User page of the dApp
//When the user address is not seen as an admin, it loads these features
function UserButtonsRevealStake (props) {  
    ...
    const _allowance = async (to) => {
        const allowance = await App1.allowance(shownAddress, to)
    ...
    const _approve = async (_value) => {
        const approve = await App1.approve(shownAddress, _value)
        setApprove(approve)
        alert('Transaction Approved')
        return approve
    }

    const shownAddress = props.userAddress
    const [Send, setSend] = useState(false)

    const _transferFrom = async (to, _value) => {
        var _Value = _value * (1E+18)
        const transferFrom = await App1.transferFrom(shownAddress, to, _Value, 
    ...
    //This is a small function using web3js to convert the UserPrivateKey to the
    ...
    //UseEffect simply tells react to automatically call getUser function each
    //time the User page is loaded
    ...
    //This returns the features visible to the User page
    return (
        <div>
            ...
            <div className='hiddenBtn'>
                <ul id='hiddenButtons'>
                    ...
                    (<UserTransfer onSendParameters={_transferFrom} onAllowance={_allowance} 
...
export default UserButtonsRevealStake